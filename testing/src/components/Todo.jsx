  ...
  return (
    <>
      <div className="w-full min-h-screen bg-gray-200 flex flex-col items-center ">
        <h1 className='text-3xl font-bold items-center text-indigo-700 m-5'>TODO LIST</h1>
        <div className="w-full flex justify-center">
          ...
          />
          <button
            className='p-3 ml-5 bg-indigo-700 hover:bg-indigo-600  text-white border rounded'
            onClick={addTodo}
          >
            Add Todo
          </button>
        ...
        <div className="w-full mt-5">
          {todos.length > 0 ?
            <select onChange={statusHandler} className=' bg-gray-300 p-2 border-none  ml-3 rounded' >
            ...
            {todos.map((item) => {
              if (status == "all" || (status == "completed" && item.check == true) || (status == "notcompleted" && item.check == false)) {
                return <li className=' w-1/2 m-2' key={item.id}>
                  ...
                  <input className={`${item.check === true ? "line-through bg-gray-500" : ""} p-2 rounded`} type="text" value={item.todo || <Skeleton />} />
                  <button onClick={() => { editHandler(item) }} className='ml-5 p-2  text-white hover:bg-green-500 bg-green-600 border  rounded' >Edit</button>
                  <button onClick={() => { deleteHandler(item.id) }} className='ml-5 p-2 text-white hover:bg-red-500 bg-red-600 border  rounded'>delete</button>
                </li>
              }
            })}
          </ol>
